...
class Test_Composer(unittest.TestCase):
    ...
    def __init__(self, methodName: str = ..., pair: str = None, fetch: bool = False, predict: bool = False,
                 box_pts: int = 10, interval: int = None, strategy: str = 'mirrored', no_request: bool = False,
                 end_time: str = None, test: bool = False, continue_training: bool = False):
    ...
    def test_composer(self):
        """Run test with asyncio to handle async methods."""
        asyncio.run(self._test_composer_async())

    async def _test_composer_async(self):
        """Actual test logic wrapped in async to handle async operations."""
        # Eliminate randomness
        from_file = not self.fetch
        ...
        # Asynchronously acquire data
        await self.composer.aquire(
            from_file=from_file, interval=self.interval, no_request=self.no_request, ignore_start=ignore_start,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--continue-training', action='store_true', default=True, help='Continue training of the model')
